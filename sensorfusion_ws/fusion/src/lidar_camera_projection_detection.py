...
class LidarCameraProjectionDetectionNode(Node):
    def __init__(self):
        ...
        # Load YOLOv8 model — replace with custom cone model when available

        self.get_logger().info("Loading YOLOv8 TensorRT engine...")

        # Point to the engine and specify the task 
        self.yolo_model = YOLO('yolov8n.engine', task='detect')

        self.get_logger().info("YOLOv8 running natively on CUDA via TensorRT.")
    ...
    def sync_callback(self, image_msg: Image, lidar_msg: PointCloud2):
        ...
        # Collect all bounding boxes from YOLO and HSV cone detection
        ...
        for result in results:
            for box in result.boxes:
                ...
                # Get closest LiDAR point inside bounding box from depth matrix
                ...
                if len(valid_depths) > 0:
                    ...
                    # Yellow box — no LiDAR depth available
        ...
        # ── Bounding Box Filtered LiDAR Dots ─────────────────────────────────
        # Only draw LiDAR points that fall inside a detected bounding box
        # This removes background noise (walls, ceiling, floor) from the display
        for i in range(len(u_proj)):
            u_int = int(u_proj[i] + 0.5)
            v_int = int(v_proj[i] + 0.5)
            if not (0 <= u_int < w and 0 <= v_int < h):
                continue
            # Check if this point falls inside any bounding box
            inside_box = any(
            ...
            if inside_box:
                ...
                cv2.circle(cv_image, (u_int, v_int), 2, (0, color_intensity, 255 - color_intensity), -1)
